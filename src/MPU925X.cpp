...
/* MPU925X object, input the I2C bus and address */
MPU925X::MPU925X(TwoWire &bus, uint8_t address)
{
    _i2c = &bus;        // I2C bus
...
/* MPU925X object, input the SPI bus and chip select pin */
MPU925X::MPU925X(SPIClass &bus, uint8_t csPin)
{
    _spi = &bus;    // SPI bus
...
int MPU925X::begin()
{
    ...
    // instruct the MPU925X to get 7 bytes of data from the AK8963 at the sample rate
...
int MPU925X::setAccelRange(AccelRange range)
{
    // use low speed SPI for register setting
    _useSPIHS = false;
    switch (range)
...
int MPU925X::setGyroRange(GyroRange range)
{
    // use low speed SPI for register setting
    _useSPIHS = false;
    switch (range)
...
int MPU925X::setDlpfBandwidth(DlpfBandwidth bandwidth)
{
    // use low speed SPI for register setting
    _useSPIHS = false;
    switch (bandwidth)
...
int MPU925X::setSrd(uint8_t srd)
{
    // use low speed SPI for register setting
    _useSPIHS = false;
...
int MPU925X::enableDataReadyInterrupt()
{
    // use low speed SPI for register setting
    _useSPIHS = false;
...
int MPU925X::disableDataReadyInterrupt()
{
    // use low speed SPI for register setting
    _useSPIHS = false;
...
int MPU925X::enableWakeOnMotion(float womThresh_mg, LpAccelOdr odr)
{
    // use low speed SPI for register setting
    _useSPIHS = false;
    // set AK8963 to Power Down
...
int MPU925XFIFO::enableFifo(bool accel, bool gyro, bool mag, bool temp)
{
    // use low speed SPI for register setting
    _useSPIHS = false;
...
/* reads the most current data from MPU925X and stores in buffer */
int MPU925X::readSensor()
{
    _useSPIHS = true; // use the high speed SPI for data readout
    // grab the data from the MPU925X
...
float MPU925X::getAccelX_mss()
{
    return _ax;
}

/* returns the accelerometer measurement in the y direction, m/s/s */
float MPU925X::getAccelY_mss()
{
    return _ay;
}

/* returns the accelerometer measurement in the z direction, m/s/s */
float MPU925X::getAccelZ_mss()
{
    return _az;
}

/* returns the gyroscope measurement in the x direction, rad/s */
float MPU925X::getGyroX_rads()
{
    return _gx;
}

/* returns the gyroscope measurement in the y direction, rad/s */
float MPU925X::getGyroY_rads()
{
    return _gy;
}

/* returns the gyroscope measurement in the z direction, rad/s */
float MPU925X::getGyroZ_rads()
{
    return _gz;
}

/* returns the magnetometer measurement in the x direction, uT */
float MPU925X::getMagX_uT()
{
    return _hx;
}

/* returns the magnetometer measurement in the y direction, uT */
float MPU925X::getMagY_uT()
{
    return _hy;
}

/* returns the magnetometer measurement in the z direction, uT */
float MPU925X::getMagZ_uT()
{
    return _hz;
}

/* returns the die temperature, C */
float MPU925X::getTemperature_C()
{
    return _t;
}

/* reads data from the MPU925X FIFO and stores in buffer */
int MPU925XFIFO::readFifo()
{
    _useSPIHS = true; // use the high speed SPI for data readout
    // get the fifo size
...
void MPU925XFIFO::getFifoAccelX_mss(size_t *size, float *data)
{
    *size = _aSize;
...
void MPU925XFIFO::getFifoAccelY_mss(size_t *size, float *data)
{
    *size = _aSize;
...
void MPU925XFIFO::getFifoAccelZ_mss(size_t *size, float *data)
{
    *size = _aSize;
...
void MPU925XFIFO::getFifoGyroX_rads(size_t *size, float *data)
{
    *size = _gSize;
...
void MPU925XFIFO::getFifoGyroY_rads(size_t *size, float *data)
{
    *size = _gSize;
...
void MPU925XFIFO::getFifoGyroZ_rads(size_t *size, float *data)
{
    *size = _gSize;
...
void MPU925XFIFO::getFifoMagX_uT(size_t *size, float *data)
{
    *size = _hSize;
...
void MPU925XFIFO::getFifoMagY_uT(size_t *size, float *data)
{
    *size = _hSize;
...
void MPU925XFIFO::getFifoMagZ_uT(size_t *size, float *data)
{
    *size = _hSize;
...
void MPU925XFIFO::getFifoTemperature_C(size_t *size, float *data)
{
    *size = _tSize;
...
void MPU925X::setAccelCalY(float bias, float scaleFactor)
{
    _ayb = bias;
    _ays = scaleFactor;
}
...
void MPU925X::setAccelCalZ(float bias, float scaleFactor)
{
    _azb = bias;
    _azs = scaleFactor;
}
...
void MPU925X::setMagCalY(float bias, float scaleFactor)
{
    _hyb = bias;
    _hys = scaleFactor;
}
...
void MPU925X::setMagCalZ(float bias, float scaleFactor)
{
    _hzb = bias;
    _hzs = scaleFactor;
}

/* writes a byte to MPU925X register given a register address and data */
int MPU925X::writeRegister(uint8_t subAddress, uint8_t data)
{
    /* write data to device */
    if (_useSPI)
    {
        _spi->beginTransaction(SPISettings(SPI_LS_CLOCK, MSBFIRST, SPI_MODE3)); // begin the transaction
        digitalWrite(_csPin, LOW);                                              // select the MPU925X chip
        _spi->transfer(subAddress);                                             // write the register address
        _spi->transfer(data);                                                   // write the data
        digitalWrite(_csPin, HIGH);                                             // deselect the MPU925X chip
...
/* reads registers from MPU925X given a starting register address, number of bytes, and a pointer to store data */
int MPU925X::readRegisters(uint8_t subAddress, uint8_t count, uint8_t *dest)
{
    if (_useSPI)
    {
...
int MPU925X::writeAK8963Register(uint8_t subAddress, uint8_t data)
{
...
int MPU925X::readAK8963Registers(uint8_t subAddress, uint8_t count, uint8_t *dest)
{
    ...
    delay(1); // takes some time for these registers to fill
              // read the bytes off the MPU925X EXT_SENS_DATA registers
    _status = readRegisters(EXT_SENS_DATA_00, count, dest);
    return _status;
}

/* gets the MPU925X WHO_AM_I register value, expected to be 0x71 */
int MPU925X::whoAmI()
{
